...
class App extends Component {
  ...
  handleBuyToken = async () => {
    ...
    await this.myTokenSale.methods
      ...
      .send({ from: this.accounts[0], value: tokenCount });
  };
  ...
  handleListItem = async () => {
    try {
      ...
      alert(
        `Item "${itemName}" listed on the marketplace for ${itemPrice} SCT.`
      );
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1 className="text-red-500">KYC CUSTOMER</h1>
        ...
        <h1>Buy CTP-Tokens</h1>
        <p>Send Ether to this address: {this.state.tokenSaleAddress}</p>
        <p>Address customer:{this.accounts[0]}</p>
        <p>You have: {this.state.userTokens}</p>
        <InputNumber
          value={this.state.tokenCount}
          onValueChange={this.handleInputBuyToken}
          useGrouping={false}
        />
        {/* <input
          type="text"
          name="tokenCount"
          value={}
          onChange={this.handleInputBuyToken}
        /> */}
        ...
        <div>
          ...
          <InputNumber
            ...
            onValueChange={this.handleItemInputChange}
            useGrouping={false}
          />
                ...
                {this.state.listings.map((item, index) => (
                  <div key={index}>
                    ...
                    {item.isActive ? (
                      <button
                        type="button"
                        onClick={() => this.handlePurchaseItem(item.listingId)}>
                        Purchase
                      </button>
                    ) : (
